...
#include <array>
namespace ashfall {
// Checked-in quarter-wave Q16.16 table; no platform libm calls in the tick path.
inline constexpr std::array<int32_t, 257> SIN_QUARTER = {
...
inline fx16 sine_phase(uint64_t tick, int period) {
  const uint64_t phase =
      (tick % static_cast<uint64_t>(period)) * 1024 * 65536 / static_cast<uint64_t>(period);
  const unsigned position = static_cast<unsigned>(phase >> 16),
                 fraction = static_cast<unsigned>(phase & 65535);
  auto sample = [](unsigned i) {
    i %= 1024;
    if (i <= 256)
      return SIN_QUARTER[i];
    if (i <= 512)
      return SIN_QUARTER[512 - i];
    if (i <= 768)
      return -SIN_QUARTER[i - 512];
    return -SIN_QUARTER[1024 - i];
  };
  const int64_t a = sample(position), b = sample(position + 1);
  return fx16::from_raw(static_cast<int32_t>(a + (b - a) * fraction / 65536));
}
inline fx16 pow_fx(fx16 v, int exponent) {
  fx16 result = FX_ONE;
